...
const CourseDetails = () => {
  ...
  return courseData ? (
    ...
    <div className='max-w-xl z-10 text-gray-500'>
     ...
     {{__html:courseData.courseDescription.slice(0,200)}}></p>


     <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
               <p>{calculateRating(courseData)}</p>
               <div className='flex'>{[...Array(5)].map((_,i)=>(<img key={i} src=
                    ...
                    {courseData.courseContent.map((chapter,index)=>(
                      <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ${openSection[index] ? 'max-h-96' : 'max-h-0'}`}>
                          <ul className='list-disc md:pl-10 pl-10 pr-4 py-2 text-gray-600 border-t border-gray-300'>
  ...
  {humanizeDuration(lecture.lectureDuration * 60 * 1000, {
    ...
    <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
    {
      ...
      <div className='p-5'>
        <div className='flex items-center gap-2'>
                      <img className='w-3.5' src={assets.time_left_clock_icon} alt='time left clock incon'/>
      ...
      <button className='md:mt-6 mt-4 w-full py-3 rounded bg-blue-600 text-white font-medium'>
        ...
        <div className="pt-6">
          ...
          <ul className='ml-4 pt-2 text-sm md:text-default list-disc text-gray-500'>
            <li>Step by Step guided access</li>
